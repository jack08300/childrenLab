		...
		<div id="list-survey" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="income" title="${message(code: 'survey.income.label', default: 'Income')}" />
					
						<g:sortableColumn property="job" title="${message(code: 'survey.job.label', default: 'Job')}" />
					
						<g:sortableColumn property="zipcode" title="${message(code: 'survey.zipcode.label', default: 'Zipcode')}" />
					
						<g:sortableColumn property="kids" title="${message(code: 'survey.kids.label', default: 'Kids')}" />

                        <g:sortableColumn property="kidsAge" title="${message(code: 'survey.kids.label', default: 'Kids Age')}" />
				...
				<g:each in="${surveyInstanceList}" status="i" var="surveyInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: surveyInstance, field: "income")}</td>
					
						<td>${fieldValue(bean: surveyInstance, field: "job")}</td>
					
						<td>${fieldValue(bean: surveyInstance, field: "zipcode")}</td>
					
						<td>${fieldValue(bean: surveyInstance, field: "kids")}</td>

                        <td>${fieldValue(bean: surveyInstance, field: "kidsAge")}</td>
